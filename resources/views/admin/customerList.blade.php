...
@include('admin.sidebar')
  ...
  <div class="bg-gray-100 flex-1 p-6 md:mt-16"> 
    ...
    <h2 class="font-bold text-lg mb-10">Recent Clients</h2>
    ...
    <tbody class="text-left text-black">
        ...
        <tr>
            <td class="w-1/4 mb-4 text-xs font-extrabold tracking-wider">
                <p class="name-1 text-black">{{ $client->name }}</p>                    
            </td>
            <td class="w-1/4 mb-4 text-xs font-extrabold tracking-wider">{{ $client->email }}</td>
            <td class="w-1/4 mb-4 text-xs font-extrabold tracking-wider">{{ $client->phone }}</td>
            <td class="w-1/4 mb-4 text-xs font-extrabold tracking-wider">{{ $client->address }}</td>
            <td class="w-1/4 mb-4 text-xs font-extrabold tracking-wider text-left">
                <div class="overflow-hidden rounded-full w-8 h-8">
                    <img src="{{ asset('storage/' . ($client->profile_picture ?? 'user_profile/default.jpg')) }}" class="w-full h-full" style="border: none;">
                </div>
            </td>
        </tr>
        @endforeach
