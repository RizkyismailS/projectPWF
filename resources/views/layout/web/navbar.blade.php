   ...
   <body>
    <div
  class="fixed top-0 left-0 z-40 flex items-center w-full bg-white shadow ud-header"
>
  <div class="container px-4 mx-auto">
    <div class="relative flex items-center justify-between -mx-4">
      ...
      <div class="flex items-center justify-between w-full px-4">
        <div>
          ...
          <nav id="navbarCollapse" class="absolute right-4 top-full hidden w-full max-w-[250px] rounded-lg  dark:bg-dark-2 py-5 shadow-lg lg:static lg:block lg:w-full lg:max-w-full lg:bg-transparent dark:lg:bg-transparent lg:py-0 lg:px-4 lg:shadow-none xl:px-6">
            <ul class="block lg:flex 2xl:ml-20">
              <li class="relative group">
                <a href="{{route('home')}}" class="flex py-2 mx-8 text-base font-medium ud-menu-scroll text-dark group-hover:text-primary lg:mr-0 lg:inline-flex lg:py-6 lg:px-0">Home</a>
              </li>
              <li class="relative group">
                <a href="/#about" class="flex py-2 mx-8 text-base font-medium ud-menu-scroll text-dark group-hover:text-primary lg:mr-0 lg:inline-flex lg:py-6 lg:px-0">About</a>
              </li>
              <li class="relative group">
                <a href="{{route('product')}}" class="flex py-2 mx-8 text-base font-medium ud-menu-scroll text-dark group-hover:text-primary lg:mr-0 lg:inline-flex lg:py-6 lg:px-0">Product</a>
              </li>
              <li class="relative group">
                <a href="/#team" class="flex py-2 mx-8 text-base font-medium ud-menu-scroll text-dark group-hover:text-primary lg:mr-0 lg:inline-flex lg:py-6 lg:px-0">Team</a>
              </li>
              <li class="relative group">
                <a href="/#contact" class="flex py-2 mx-8 text-base font-medium ud-menu-scroll text-dark group-hover:text-primary lg:mr-0 lg:inline-flex lg:py-6 lg:px-0">Contact</a>
              </li>
              <li class="relative group">
                <a href="product-grid.html" class="flex py-2 mx-8 text-base font-medium ud-menu-scroll text-dark group-hover:text-primary lg:mr-0 lg:inline-flex lg:py-6 lg:px-0">Products</a>
              </li>
            </ul>
          </nav>
        </div>
        ...
        <div class="relative group ml-20">
            <button id="profileDropdownBtn" class="flex items-center focus:outline-none">
                <img src="{{ Auth::user()->profile_picture ?? asset('assets/images/user_profile/default.svg') }}"
                     alt="Profile"
                     class="w-10 h-10 rounded-full object-cover border border-gray-300 ml-2" />
                <span class="ml-5 text-base font-semibold text-dark dark:text-white">
                    {{ Auth::user()->name }}
                </span>
            ...
            <div id="profileDropdownMenu" class="hidden group-hover:block absolute right-0 mt-2 w-48 bg-white dark:bg-dark-2 rounded-md shadow-lg z-50">
                <a href="{{route('userSetting')}}" class="block px-4 py-2 text-sm text-dark dark:text-white hover:bg-gray-100 dark:hover:bg-dark-3">Setting</a>
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="w-full text-left px-4 py-2 text-sm text-dark dark:text-white hover:bg-gray-100 dark:hover:bg-dark-3">Logout</button>
                </form>
            </div>
        </div>
        <script>
    ...
    @else
        <a href="{{ route('login') }}" class="loginBtn py-2 px-[22px] text-base font-medium text-dark dark:text-white hover:opacity-70">Sign In</a>
        <a href="{{ route('register') }}" class="px-6 py-2 text-base font-medium text-white duration-300 ease-in-out rounded-md signUpBtn bg-primary hover:bg-blue-dark">Sign Up</a>
    @endauth
</div>
            </div>
          </div>
